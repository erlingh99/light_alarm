...
@alarm_router.get("", response_model=List[Alarm])
def get_alarms() -> List[Alarm]:
    return list(alarms_db.values())

@alarm_router.get("/{alarm_id}", response_model=Alarm)
...
@alarm_router.post("", response_model=Alarm)
...
@alarm_router.put("/{alarm_id}", response_model=Alarm)
